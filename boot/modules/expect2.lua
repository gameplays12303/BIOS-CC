...
local function type(value,return_type)
    local FoundType = originType(value)
    if FoundType == "table" and return_type
    then
        local bool,meta = pcall(getmetatable,value)
        return bool and meta and meta.__type or "table"
    end
    return FoundType
end
...
function handle.expect(_bClasses,index,var,...)
    ...
    local name  = get_name()
    error(("argument #%s%s expected %s: got %s"):format(index,name and (" from '%s'"):format(name) or "",listerror({...}),type(var,_bClasses)),2)
end 
