...
local expect = expect or (require and require("modules.expect2") or dofile("boot/modules/expect2.lua"))
...
function utilties.string.Serialize(_data,dump_fn)
      ...
      actions["table"] = function ()
            local currentState = {depth_Pattern = "\t",depth_number = 1}
            do
                local iter,state ,key = pairs(_data)
                  currentState.iter = iter
            ...
            while currentState do
                  local key, value = currentState.iter(currentState.state, currentState.key)
                  currentState.key = key
                  if key == nil then
                        result = result .. ("%s}"):format(("\t"):rep(currentState.depth_number - 1)).."\n"
                        if currentState.target then
                              active[currentState.target] = nil
                        end
                        currentState = currentState.Parent
                  else
                        result = result .. currentState.depth_Pattern
                        -- Format key
                        if type(key) == "string" and not string.match(key, "^[%a_][%w_]*$") then
                        ...
                        if type(value) == "table" then
                              if active[value] then
                                    ...
                                    local newState = {
                                          Parent = currentState,
                                          depth_number = currentState.depth_number+1,
                                          depth_Pattern = ("\t"):rep(currentState.depth_number+1),
                                          iter = pairs(value),
                                          state = (getmetatable(value) and getmetatable(value).__pairs and select(2, getmetatable(value).__pairs(value))) or value,
                                          key = nil,
                                          target = value
                                    }
                                    result = result .. "{\n"
                              ...
                              local action = actions[type(value)]
...
---@param _sData string
---@return unknown
function utilties.string.UnSerialize(_sData,_Env,named)
      expect(false,1, _sData, "string")
      expect(false,2,_Env,"table","nil")
      expect(false,3,named,"string","nil")
      local func,err = hardWare.load("return " .. _sData, named or "unserialize","t",_Env or {})
      if func then
