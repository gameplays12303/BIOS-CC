...
local util = require and require("modules.utilities") or dofile("boot/modules/utilities.lua",_ENV)
local expect = require and require("modules.expect2") or dofile("boot/modules/expect2.lua",_ENV)
local fm = require and require("modules.fm") or dofile("boot/modules/fm.lua",_ENV)
...
},{__index = terminal,__ProtectMeta = true,access = {terminal,button,progress_bar,canvas,textBox,util,expect_utils}})
...
if ProtectMeta
then
    ProtectMeta(GUI,expect_utils,util,terminal,textBox,button,progress_bar,interactive)
end
...
function terminal:reSize(SizeX,SizeY)
    expect(false,1,SizeX,"number")
    expect(false,2,SizeY,"number")
    local meta = getmetatable(self)
    local Parent = meta.Parent
    self.window.width = SizeX
    self.window.height = SizeY
    Parent:redraw()
    return true
end
        ...
        for CurrentY = 1,SizeY do
            ...
            then
                for CurrentX = 1,SizeX do
                    ...
                    if CurrentColor
                    then
                        for iD,Color in pairs(tColourLookup) do
...
function textBox:redraw()
    ...
    then
        local nativeColor = native.getBackgroundColor()
        local nativeTextColor = native.getTextColor()
        ...
        native.setBackgroundColor(nativeColor)
        native.setTextColor(nativeTextColor)
    end
end
...
function button:setText(sText)
    ...
    then
        error(("error text too big,%s:%d<%d"):format(sText,terminalLength-2,#sText),3)
    end
    self.text = sText
end
        ...
        parallel.waitForAny(
            ...
            function ()
                ...
                while self.bool_run do
                    local event = table.pack(BIOS.pullEventRaw())
...
function terminal:run_list(option_list,TblSettings) 
    ...
    do -- sperates the tbl into pages
        ...
        for i,v in pairs(option_list) do
            ...
            Pages[PagesCount][1]:OverRide_status(true)
            Cy = Cy + 1
        end
    end
    ...
    then
        Prompt = self:create(3,1,termSizeX-4,1,true)
