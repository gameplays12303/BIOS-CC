...
local expect_table = require and require("modules.expect2") or dofile("boot/modules/expect2.lua")
...
local counter = false -- this prevents infinite recursion in case the table in question has a __pairs method that needs metatable access
local debug = util.table.copy(debug)
local function hasAccess(metaData)
    if counter or not debug
    then
        return true
    end
    local search
...
do -- load
    ...
    _G.debug.setmetatable = function (Table)
        expect(false,1,Table,"table")
        local meta = metaTable.getmetatable(Table) or {}
        if (meta.__ProtectMeta) and not hasAccess(meta)
        then
    ...
    _G.debug.getmetatable = function (Table,_metaTable)
        expect(false,1,Table,"table")
        expect(false,2,_metaTable,"table","nil")
        local meta = metaTable.getmetatable(Table) or {}
        if meta.__ProtectMeta and not hasAccess(meta)
...
local nativeLoad = HardWare.load
HardWare.load = function (x, name, mode, env)
...
if loadstring
then
    local nativeloadstring = HardWare.loadstring
    HardWare.loadstring = function (chunk,chunk_name)
